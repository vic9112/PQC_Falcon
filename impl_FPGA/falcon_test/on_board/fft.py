...
from ip import ip1, Buffer1_A, Buffer1_B
from get_vector import get_fft1024, get_ifft1024
...
#        print("not logn")
    ...
    if (n == 1024):
        ...
        start = time()
        ip1.write(0x28, 0x00)
        ip1.write(0x00, 0x01) # 0x00: bit 0: 0->1: start
        while (ip1.read(0x00) & 0x4) == 0x0:
            continue
        end = time()
        #print(f"FFT IP execution time: {end - start} sec")
        tmp = [i for i in Buffer1_A]
...
def ifft(f_fft):
    ...
    if (n == 1024):
        ...
        #print(f"iFFT IP execution time: {end - start} sec")
        f = [i for i in Buffer1_A]
    elif (n > 2):
...
def mul(f, g):
    ...
    #print(f"FFT mul execution time: {end - start} sec")
...
def adj(f):
    ...
    #print(f"FFT adj execution time: {end - start} sec")
    tmp = [0 for i in range(1024)]
    for i in range(1024):
        tmp[i] = Buffer1_A[i]
    return tmp
