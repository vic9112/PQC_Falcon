...
from ip import ip0, Buffer0_A, Buffer0_B
from get_vector import get_ntt, get_intt
...
def ntt(f):
    ...
    if (n == 1024):
        for i in range(n):
            if f[i] < 0:
                Buffer0_A[i] = f[i] + 12289
            else:
                Buffer0_A[i] = f[i]
    
        start = time()
        ip0.write(0x28, 0x02)
        ip0.write(0x00, 0x01) # 0x00: bit 0: 0->1: start
        while (ip0.read(0x00) & 0x4) == 0x0:
            continue
        end = time()
        #print(f"NTT IP execution time: {end - start} sec")
        tmp = [int(i) for i in Buffer0_A]
        f_ntt = get_ntt(tmp)
    elif (n > 2):
        f0, f1 = split(f)
        f0_ntt = ntt(f0)
...
def intt(f_ntt):
    ...
    if (n == 1024):
        ...
        #print(f"iNTT IP execution time: {end - start} sec")
        f = [int(i % q) for i in Buffer0_A]
...
def div_ntt(f_ntt, g_ntt):
    ...
    return [(f_ntt[i] * inv_mod_q[int(g_ntt[i])]) % q for i in range(deg)]
