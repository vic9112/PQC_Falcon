...
void test_iFFT(void) {
	...
	for (int i = 0; i < SIZE; i++)
		in_fft[i].f = FFT_in[i];
	fiFFNTT(in_fft,0, fft_req, 0,0,0); // FFT
	for (int j = 0; j < SIZE; j++)
		fft_out[j] = in_fft[j].f;

	for (int k = 0; k < SIZE; k++)
		in_ifft[k].f = fft_out[k];
	fiFFNTT(in_ifft,0, fft_req, 1,0,0); // iFFT
...
void test_iNTT(void) {
	...
	fiFFNTT(in_ntt,0, ntt_req, 0,0,0); // NTT
	for (int j = 0; j < SIZE; j++)
		ntt_out[j] = in_ntt[j].u16.n0;

	for (int k = 0; k < SIZE; k++)
		in_intt[k].u16.n0 = ntt_out[k];
	fiFFNTT(in_intt,0, ntt_req, 1,0,0); // iNTT
	for (int l = 0; l < SIZE; l++)
...
void test_adj(void){
		...
		for (int i = 0; i < SIZE; i++)
			in_fft[i].f = py_f[i];
		fiFFNTT(in_fft,0, fft_req, 0,1,0); // FFT
		for (int j = 0; j < SIZE; j++)
			fft_out[j] = in_fft[j].f;
...
void test_mul(void){
		...
		fiFFNTT(in1_fft,in2_fft, fft_req, 0,0,1); // FFT
		for (int l = 0; l < SIZE; l++)
