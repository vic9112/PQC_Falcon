...
void streamtoparallelwithburst(hls::stream<data> &in_stream, hls::stream<int> &in_counts, bool &buf_sts,
 ap_uint<1> clr_sts, ap_uint<2> kernel_mode, memcell *out_memory) {
 ...
 VITIS_LOOP_18_1: do {
  ...
  high = 0;
  int a = 0;
  VITIS_LOOP_22_2: for (int i = 0; i < count; ++i) {
#pragma HLS PIPELINE
...
void paralleltostreamwithburst(memcell *in_memory, ap_uint<2> kernel_mode, hls::stream<data> &out_stream) {
 ...
 VITIS_LOOP_112_1: do {
  ...
  VITIS_LOOP_121_2: for (int i = 0; i < count; ++i) {
#pragma HLS PIPELINE
...
void sendoutstream(hls::stream<data> &in_stream, bool &buf_sts, ap_uint<1> clr_sts,
...
#pragma HLS INTERFACE s_axilite port=clr_s2m_sts bundle=control
#pragma HLS INTERFACE s_axilite port=clr_m2s_sts bundle=control
...
#pragma HLS DATAFLOW
 ...
 streamtoparallelwithburst(inbuf, incount, *s2m_buf_sts, clr_s2m_sts, kernel_mode, s2mbuf);
 paralleltostreamwithburst(m2sbuf, kernel_mode, outbuf);
 sendoutstream(outbuf, *m2s_buf_sts, clr_m2s_sts, outStreamTop);
