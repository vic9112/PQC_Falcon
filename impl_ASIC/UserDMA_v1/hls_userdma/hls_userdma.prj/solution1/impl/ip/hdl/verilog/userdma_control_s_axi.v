...
#(parameter
    ...
    output wire [0:0]                    clr_s2m_sts,
    output wire [0:0]                    clr_m2s_sts,
...
localparam
    ...
    ADDR_IER                = 7'h08,
    ADDR_ISR                = 7'h0c,
    ADDR_KERNEL_MODE_DATA_0 = 7'h10,
    ADDR_KERNEL_MODE_CTRL   = 7'h14,
    ADDR_S2M_BUF_STS_DATA_0 = 7'h18,
    ADDR_S2M_BUF_STS_CTRL   = 7'h1c,
    ADDR_M2S_BUF_STS_DATA_0 = 7'h28,
    ADDR_M2S_BUF_STS_CTRL   = 7'h2c,
    ADDR_S2MBUF_DATA_0      = 7'h38,
    ADDR_S2MBUF_DATA_1      = 7'h3c,
    ADDR_S2MBUF_CTRL        = 7'h40,
    ADDR_M2SBUF_DATA_0      = 7'h44,
    ADDR_M2SBUF_DATA_1      = 7'h48,
    ADDR_M2SBUF_CTRL        = 7'h4c,
    ADDR_S2M_ERR_DATA_0     = 7'h50,
    ADDR_S2M_ERR_CTRL       = 7'h54,
    ADDR_CLR_S2M_STS_DATA_0 = 7'h60,
    ADDR_CLR_S2M_STS_CTRL   = 7'h64,
    ADDR_CLR_M2S_STS_DATA_0 = 7'h68,
    ADDR_CLR_M2S_STS_CTRL   = 7'h6c,
    WRIDLE                  = 2'd0,
    WRDATA                  = 2'd1,
    WRRESP                  = 2'd2,
    WRRESET                 = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [0:0]                    int_clr_s2m_sts = 'b0;
    reg  [0:0]                    int_clr_m2s_sts = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_KERNEL_MODE_DATA_0: begin
                    rdata <= int_kernel_mode[1:0];
                end
                ...
                ADDR_CLR_S2M_STS_DATA_0: begin
                    rdata <= int_clr_s2m_sts[0:0];
                end
                ADDR_CLR_M2S_STS_DATA_0: begin
                    rdata <= int_clr_m2s_sts[0:0];
                end
            endcase
        end
    end
...
assign kernel_mode       = int_kernel_mode;
assign s2mbuf            = int_s2mbuf;
assign m2sbuf            = int_m2sbuf;
assign clr_s2m_sts       = int_clr_s2m_sts;
assign clr_m2s_sts       = int_clr_m2s_sts;
// int_interrupt
always @(posedge ACLK) begin
    if (ARESET)
        int_interrupt <= 1'b0;
...
always @(posedge ACLK) begin
    if (ARESET)
        int_kernel_mode[1:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_KERNEL_MODE_DATA_0)
            int_kernel_mode[1:0] <= (WDATA[31:0] & wmask) | (int_kernel_mode[1:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_clr_s2m_sts[0:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_CLR_S2M_STS_DATA_0)
            int_clr_s2m_sts[0:0] <= (WDATA[31:0] & wmask) | (int_clr_s2m_sts[0:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_clr_m2s_sts[0:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_CLR_M2S_STS_DATA_0)
            int_clr_m2s_sts[0:0] <= (WDATA[31:0] & wmask) | (int_clr_m2s_sts[0:0] & ~wmask);
    end
end
