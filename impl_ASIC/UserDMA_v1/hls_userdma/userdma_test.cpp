...
void test_NTT (memcell s2m_buf[BUF_LEN], memcell m2s_buf[BUF_LEN], memcell result[BUF_LEN]) {
	...
	bool m2s_buf_sts = 0;
	ap_uint<2> s2m_err;
	ap_uint<1> clr_s2m_sts = 1;
	ap_uint<1> clr_m2s_sts = 1;
...
void test_FFT (memcell s2m_buf[BUF_LEN], memcell m2s_buf[BUF_LEN], memcell result[BUF_LEN]) {
	...
	userdma(inStream_t, outStream_t, 0, &s2m_buf_sts, &m2s_buf_sts, s2m_buf, m2s_buf, &s2m_err,  clr_s2m_sts, clr_m2s_sts);
