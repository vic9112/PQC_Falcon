...
void XFft_Set_f(XFft *InstancePtr, u64 Data) {
    Xil_AssertVoid(InstancePtr != NULL);
    Xil_AssertVoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    XFft_WriteReg(InstancePtr->Control_BaseAddress, XFFT_CONTROL_ADDR_F_DATA, (u32)(Data));
    XFft_WriteReg(InstancePtr->Control_BaseAddress, XFFT_CONTROL_ADDR_F_DATA + 4, (u32)(Data >> 32));
}

u64 XFft_Get_f(XFft *InstancePtr) {
    u64 Data;

    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    Data = XFft_ReadReg(InstancePtr->Control_BaseAddress, XFFT_CONTROL_ADDR_F_DATA);
    Data += (u64)XFft_ReadReg(InstancePtr->Control_BaseAddress, XFFT_CONTROL_ADDR_F_DATA + 4) << 32;
    return Data;
}

void XFft_Set_logn(XFft *InstancePtr, u32 Data) {
    Xil_AssertVoid(InstancePtr != NULL);
    Xil_AssertVoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    XFft_WriteReg(InstancePtr->Control_BaseAddress, XFFT_CONTROL_ADDR_LOGN_DATA, Data);
}

u32 XFft_Get_logn(XFft *InstancePtr) {
    u32 Data;

    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    Data = XFft_ReadReg(InstancePtr->Control_BaseAddress, XFFT_CONTROL_ADDR_LOGN_DATA);
    return Data;
}
