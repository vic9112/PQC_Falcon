...
entity fiFFNTT_control_s_axi is
generic (
...
port (
    ...
    inverse               :out  STD_LOGIC_VECTOR(0 downto 0);
    ap_start              :out  STD_LOGIC;
    ap_done               :in   STD_LOGIC;
    ap_ready              :in   STD_LOGIC;
    ap_idle               :in   STD_LOGIC
);
end entity fiFFNTT_control_s_axi;
...
architecture behave of fiFFNTT_control_s_axi is
    ...
    constant ADDR_MODE_DATA_0    : INTEGER := 16#1c#;
    constant ADDR_MODE_CTRL      : INTEGER := 16#20#;
    constant ADDR_INVERSE_DATA_0 : INTEGER := 16#24#;
    constant ADDR_INVERSE_CTRL   : INTEGER := 16#28#;
    ...
    signal int_inverse         : UNSIGNED(0 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_INVERSE_DATA_0 =>
                        rdata_data <= RESIZE(int_inverse(0 downto 0), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    inverse              <= STD_LOGIC_VECTOR(int_inverse);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_INVERSE_DATA_0) then
                    int_inverse(0 downto 0) <= (UNSIGNED(WDATA(0 downto 0)) and wmask(0 downto 0)) or ((not wmask(0 downto 0)) and int_inverse(0 downto 0));
                end if;
            end if;
        end if;
    end process;
