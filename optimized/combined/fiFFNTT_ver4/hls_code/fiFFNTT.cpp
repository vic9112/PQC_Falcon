...
#pragma HLS INLINE off
   ...
   //#pragma HLS UNROLL
		out_buf[j].f=Negate(out_buf[j].f);
	}
	...
	out_copy(in_buf, in, 1, 1);}
	    ...
	    for (int k = 0; k < 512; k++)
		in_buf[k].f = d_add(d_mul(out_buf[k].f,out_buf2[k].f),Negate(d_mul(out_buf[k+512].f,out_buf2[k+512].f)));
	    for (int l = 512; l< 1024; l++)
		in_buf[l].f = d_add(d_mul(out_buf[l].f,out_buf2[l-512].f),d_mul(out_buf[l-512].f,out_buf2[l].f));
