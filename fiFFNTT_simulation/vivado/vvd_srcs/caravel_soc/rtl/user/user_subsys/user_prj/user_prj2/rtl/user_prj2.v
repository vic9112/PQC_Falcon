...
always@(*)begin
  case(in_state)
    Command: if(ss_tvalid && ss_tdata[31:30]==2'b10 && (ss_tdata[29:28]==2'd0 || ss_tdata[29:28]== 2'd1))  next_in_state = F_IN1;
             else if(ss_tvalid && ss_tdata[31:30]==2'b10 && (ss_tdata[29:28]==2'd2 || ss_tdata[29:28]==2'd3))  next_in_state = U_IN;
             else next_in_state=Command;
    F_IN1:   if(ss_tvalid)next_in_state = F_IN2;
    		else next_in_state = F_IN1;
    F_IN2:   if(In_rdy)next_in_state = F_IN1;
    		else next_in_state = F_IN2;
    U_IN:    if(In_copy_done)next_in_state = Command;
    	     else next_in_state = U_IN;
    default:next_in_state=Command;
end 
...
assign ss_tready=(in_state==U_IN|| in_state== F_IN2)?In_rdy:(in_state==F_IN1)?1'b1:0;
assign In_data=(in_state==U_IN)?{32'b0,ss_tdata}:{ss_tdata,regx_data};
assign In_vld=(in_state==U_IN)?ss_tvalid:0;
*/

reg [15:0]reg_mode1_in;
assign ss_tready=(state==Command)?1'b1:(state==IN_COPY)?In_rdy:1'b0;
//assign ss_tready=(state==IN_COPY)?In_rdy:1'b0;
