...
from sign_KAT import sign_KAT1024
...
class KeyGen:
    ...
    def __init__(self, n, i, polys=None):
        """Initialize a secret key and generate a public key"""
        # Public parameters
        self.n = n
        ...
        if polys is None:
            self.f = sign_KAT1024[i]['f']
            self.g = sign_KAT1024[i]['g']
            self.F = sign_KAT1024[i]['F']
            self.G = sign_KAT1024[i]['G']
        else:
            [f, g, F, G] = polys
...
class Sign:
    ...
    - sign a message
    """
    def __init__(self, message, key, randombytes=urandom):
        self.n = key.n
        self.h = key.h # public key
        self.sigma = Params[key.n]["sigma"]
        self.sigmin = Params[key.n]["sigmin"]
        self.signature_bound = Params[key.n]["sig_bound"]
        self.sig_bytelen = Params[key.n]["sig_bytelen"]
        
        # From f, g, F, G, compute the basis B0 of a NTRU lattice
        # as well as its Gram matrix and their fft's.
        B0 = [[key.g, neg(key.f)], [key.G, neg(key.F)]]
        G0 = gram(B0)
        ...
        self.sign = self.signature(message, randombytes)
	
    def __repr__(self):
        """Print the object in readable form."""
        rep = "Signature: {result}".format(result=self.sign)
        return rep
    ...
    def signature(self, message, randombytes=urandom):
        """
        Sign a message. The message MUST be a byte string or byte array.
        Optionally, one can select the source of (pseudo-)randomness used
        (default: urandom).
        ...
        hashed = hash_to_point(self.n, message, salt)
...
class Verify:
    ...
    """
    def __init__(self, message, sig):
        self.n = sig.n
        self.h = sig.h
        self.signature_bound = sig.signature_bound
        self.bytelen = sig.sig_bytelen
        signature = sig.sign
        # Unpack the salt and the short polynomial s1
        self.vrfy = self.verify(message, signature)
            
    def __repr__(self):
        """Print the object in readable form."""
        rep = "Verification Result: {result}".format(result=self.vrfy)
        return rep
    ...
    def verify(self, message, signature):
        ...
        s1 = decompress(enc_s, self.bytelen - HEAD_LEN - SALT_LEN, self.n)
        ...
        norm_sign = sum(np.uint16(coef ** 2) for coef in s0)
        norm_sign += sum(np.uint16(coef ** 2) for coef in s1)
        if norm_sign > self.signature_bound:
            print("Squared norm of signature is too large:", norm_sign, self.signature_bound)
