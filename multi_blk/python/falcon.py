...
from ip import ipComputePK, inBufferg, inBufferf, outBufferh
...
def compute_pk_ip(g, f):
    ...
    for j in range(len(f)):
        inBufferf[j] = f[j]
    logn = int(log2(len(g)))
    ipComputePK.write(0x3C, logn) # Coefficient
    ipComputePK.write(0x18, outBufferh.device_address)  # get h
    ipComputePK.write(0x24, inBufferf.device_address)  # write f
    ipComputePK.write(0x30, inBufferg.device_address)  # write g
    # =================== Start ======================
    ipComputePK.write(0x00, 0x01) # 0x00: bit 0: 0->1: start
    while (ipComputePK.read(0x00) & 0x4) == 0x0: # wait for ap_done
        continue
    timeKernelEnd = time()
    print("Compute public key IP execution time: " + str(timeKernelEnd - timeKernelStart) + " s")
    h = outBufferh[:]
    return h
...
# =============== Replace it with IP compute_public =================
# ----> self.h = div_zq(self.g, self.f)
        self.h = compute_pk_ip(self.g, self.f)
# ===================================================================
