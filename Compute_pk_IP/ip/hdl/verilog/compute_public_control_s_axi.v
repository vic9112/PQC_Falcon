...
// ==============================================================
`timescale 1ns/1ps
module compute_public_control_s_axi
#(parameter
    ...
    output wire [63:0]                   f,
    output wire [63:0]                   g,
    output wire [31:0]                   logn,
    output wire [63:0]                   tmp
);
//------------------------Address Info-------------------
...
localparam
    ...
    ADDR_G_DATA_1    = 7'h34,
    ADDR_G_CTRL      = 7'h38,
    ADDR_LOGN_DATA_0 = 7'h3c,
    ADDR_LOGN_CTRL   = 7'h40,
    ADDR_TMP_DATA_0  = 7'h44,
    ADDR_TMP_DATA_1  = 7'h48,
    ADDR_TMP_CTRL    = 7'h4c,
    WRIDLE           = 2'd0,
    WRDATA           = 2'd1,
    WRRESP           = 2'd2,
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_LOGN_DATA_0: begin
                    rdata <= int_logn[31:0];
                end
                ADDR_TMP_DATA_0: begin
                    rdata <= int_tmp[31:0];
                end
                ADDR_TMP_DATA_1: begin
                    rdata <= int_tmp[63:32];
                end
            endcase
        end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_LOGN_DATA_0)
            int_logn[31:0] <= (WDATA[31:0] & wmask) | (int_logn[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_TMP_DATA_0)
            int_tmp[31:0] <= (WDATA[31:0] & wmask) | (int_tmp[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_TMP_DATA_1)
            int_tmp[63:32] <= (WDATA[31:0] & wmask) | (int_tmp[63:32] & ~wmask);
    end
end
