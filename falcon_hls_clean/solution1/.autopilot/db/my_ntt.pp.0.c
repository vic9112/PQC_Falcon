...
#pragma HLS DATAFLOW
 ...
 n = (size_t)1 << logn;
 t = n;
 VITIS_LOOP_150_1: for (m = 1; m < n; m <<= 1) {
  size_t ht, i, j1;

  ht = t >> 1;
  VITIS_LOOP_154_2: for (i = 0, j1 = 0; i < m; i ++, j1 += t) {
   size_t j, j2;
   uint32_t s;

   s = GMb[m + i];
   j2 = j1 + ht;
   VITIS_LOOP_160_3: for (j = j1; j < j2; j ++) {
    uint32_t u, v;
    uint32_t tmp1, tmp2;
