...
# 2 "<built-in>" 2
# 1 "hls_source/keygen.c" 2
# 32 "hls_source/keygen.c"
...
1, 0, 0, 1, 0, 0, 4, 3, 2, 1, 0, 0, 1, 0, 0, 2, 1, 0, 0, 1, 0, 0, 3, 2, 1, 0, 0, 1, 0, 0,
2, 1, 0, 0, 1, 0, 0};
# 33 "hls_source/keygen.c" 2
# 95 "hls_source/keygen.c"
typedef struct {
 uint32_t p;
 uint32_t g;
 uint32_t s;
...
# 2059 "hls_source/keygen.c"
  ...
  a_lo = a[0];
  b_lo = b[0];
# 2103 "hls_source/keygen.c"
  pa = 1;
  pb = 0;
  qa = 0;
  qb = 1;
  VITIS_LOOP_2107_3: for (i = 0; i < 31; i ++) {
# 2120 "hls_source/keygen.c"
 ...
 uint32_t sch, uint32_t scl)
{
 ...
 VITIS_LOOP_2225_1: for (u = sch; u < xlen; u ++) {
  ...
  x[u] = (uint32_t)z & 0x7FFFFFFF;
# 2254 "hls_source/keygen.c"
  ccu = (uint32_t)(z >> 31);
  cc = *(int32_t *)&ccu;
 }
}
# 2270 "hls_source/keygen.c"
...
# 3076 "hls_source/keygen.c"
 if (!zint_bezout(Gp, Fp, fp, gp, len, t1)) {
  return 0;
 }
# 3087 "hls_source/keygen.c"
 q = 12289;
...
# 3464 "hls_source/keygen.c"
 FGlen = llen;
 maxbl_FG = 31 * (int)llen;
# 3492 "hls_source/keygen.c"
 ...
 solve_NTRU_intermediate_loop3: for (;;) {
  ...
  iFFT(rt2, logn);
# 3533 "hls_source/keygen.c"
  dc = scale_k - scale_FG + scale_fg;
# 3543 "hls_source/keygen.c"
  if (dc < 0) {
   dc = -dc;
   pt = fpr_two;
  } else {
...
# 4523 "hls_source/keygen.c"
    if(depth == 10){
        ...
        zint_rebuild_CRT(fp, len, len, 2, primes, 0, t1);
# 4554 "hls_source/keygen.c"
        if (!zint_bezout(Gp, Fp, fp, gp, len, t1)) {
   tmp2[0] = 7;
            return 0;
        }
# 4566 "hls_source/keygen.c"
        q = 12289;
        if (zint_mul_small(Fp, len, q) != 0
            || zint_mul_small(Gp, len, q) != 0)
        {
...
# 5502 "hls_source/keygen.c"
 ...
 poly_adj_fft(rt3, logn);
 poly_adj_fft(rt4, logn);
# 5537 "hls_source/keygen.c"
 FGlen = llen;
 maxbl_FG = 31 * (int)llen;
# 5565 "hls_source/keygen.c"
 scale_k = maxbl_FG - minbl_fg;
 ...
 solve_NTRU_intermediate_loop3: for (;;) {
  ...
  poly_mul_autoadj_fft(rt2, rt5, logn);
  iFFT(rt2, logn);
# 5606 "hls_source/keygen.c"
  dc = scale_k - scale_FG + scale_fg;
# 5616 "hls_source/keygen.c"
  if (dc < 0) {
   dc = -dc;
   pt = fpr_two;
  } else {
