...
__attribute__((sdx_kernel("mq_iNTT", 0))) void mq_iNTT(uint16_t *a, unsigned logn);
...
__attribute__((sdx_kernel("mq_iNTT", 0))) void mq_iNTT(uint16_t *a, unsigned logn)
{
#line 27 "/home/ubuntu/Desktop/Vic_PQC_Falcon/falcon_hls_clean/solution1/csynth.tcl"
#pragma HLSDIRECTIVE TOP name=mq_iNTT
# 140 "hls_source/my_intt.c"
...
#pragma HLS DATAFLOW
 ...
 t = 1;
 m = n;
 VITIS_LOOP_154_1: while (m > 1) {
  size_t hm, dt, i, j1;

  hm = m >> 1;
  dt = t << 1;
  VITIS_LOOP_159_2: for (i = 0, j1 = 0; i < hm; i ++, j1 += dt) {
   size_t j, j2;
   uint32_t s;

   j2 = j1 + t;
   s = iGMb[hm + i];
   VITIS_LOOP_165_3: for (j = j1; j < j2; j ++) {
    uint32_t u, v, w;
 ...
 ni = 4091;
 VITIS_LOOP_194_4: for (m = n; m > 1; m >>= 1) {

  ni += 12289 & -(ni & 1);
  ni = (ni >> 1);
 }
 VITIS_LOOP_199_5: for (m = 0; m < n; m ++) {
